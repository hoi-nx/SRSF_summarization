...
# run with main.py
class SRSF_RNN_RNN(BasicModule):
    def __init__(self, args, embed=None):
        super(SRSF_RNN_RNN, self).__init__(args)
        self.model_name = 'SRSF_RNN_RNN'
        self.args = args
        V = args.embed_num
        D = args.embed_dim
        H = args.hidden_size
        ...
        self.sent_features = nn.Linear(8, 1, bias=False)
    ...
    def forward(self, x, sent_features, doc_lens):
        ...
        for index, doc_len in enumerate(doc_lens):
            ...
            for position, h in enumerate(valid_hidden):
                ...
                my_feature = self.sent_features(m_sent_features.view(1, -1))
                prob = F.sigmoid(my_feature + content + salience + novelty + abs_p + rel_p + self.bias)
                s = s + torch.mm(prob, h)
                probs.append(prob)

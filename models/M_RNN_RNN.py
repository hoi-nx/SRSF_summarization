...
class RNN_RNN(BasicModule):
    def __init__(self, args, embed=None):
        ...
        self.ranking_sent = nn.Linear(1, 1, bias=False)
        self.cosine_first_sent = nn.Linear(1, 1, bias=False)
        self.cosine_similarity_sentence_doc = nn.Linear(1, 1, bias=False)
    ...
    def sentence_position(self,sentences):
        th = 0.2
    ...
    def forward(self, x, doc_lens):
        ...
        for index, doc_len in enumerate(doc_lens):
            ...
            for position, h in enumerate(valid_hidden):
                ...
                print(cosine_similarity.view(1, -1))
                print(h.size())
                h = h.view(1, -1)  # (1,2*H)
                ...
                ranks_sentent = self.ranking_sent(tensor_ranks.view(1,-1))
                ...
                prob = F.sigmoid(
                    ranks_sentent + cosine_with_doc + cosine_first + content + salience + novelty + abs_p + rel_p + self.bias)
                print("Prob============")
                print(prob)
                s = s + torch.mm(prob, h)
                probs.append(prob)
